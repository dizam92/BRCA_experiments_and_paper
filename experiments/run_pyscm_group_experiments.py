...
class GenericGroupScmTcga(object):
    def __init__(self, data_path, function_hps, function_name, random_state_list, saving_repertory, saving_file="",
        ...
        self.X, self.y, self.features_names, self.patients_names = load_data(data=data_path,
                                                                             return_views=self.return_views)
        self.groups_ids = []
    ...
    def learning(self):
        for cpt, indices in enumerate(self.random_state_list):
            ...
            # TODO: I have the intuition along the SCM basic result that it's always the conjunction that is picked.
            ...
            self.saving_dict[indices]['groups_ids'] = self.groups_ids
            print('Saving the groups ids\n')
            for el in self.saving_dict[indices]['rules']:
                print(el.feature_idx)
                print(self.groups_ids_weights[el.feature_idx])
            print('-------------------------------\n')
            self.saving_dict[indices]['groups_ids_weights_rules_str'] = [self.groups_ids_weights[el.feature_idx]
                                                                 for el in self.saving_dict[indices]['rules']]
            print('Saving the groups_ids_rules_str ids\n')
            for el in self.saving_dict[indices]['rules']:
                print(el.feature_idx)
                print(self.groups_ids[el.feature_idx])
