        ...
        for dataset in datasets_new_labels:
            if dataset.find("mean") != -1:
                run_experiment(data=dataset,
                               experiment_name='experiment_tn_new_label_mean',
                               return_views=view,
                               nb_repetitions=15,
                               saving_rep=saving_repository)
            if dataset.find("median") != -1:
                run_experiment(data=dataset,
                               experiment_name='experiment_tn_new_label_median',
                               return_views=view,
                               nb_repetitions=15,
                               saving_rep=saving_repository)
            if dataset.find("zero") != -1:
                run_experiment(data=dataset,
                               experiment_name='experiment_tn_new_label_zero',
                               return_views=view,
                               nb_repetitions=15,
                               saving_rep=saving_repository)
...
def main_run_experiments_old_labels():
    for view in return_views:
        for dataset in datasets_old_labels:
            if dataset.find("mean") != -1:
                run_experiment(data=dataset,
                               experiment_name='experiment_tn_old_label_mean',
                               return_views=view,
                               nb_repetitions=15,
                               saving_rep=saving_repository)
            if dataset.find("median") != -1:
                run_experiment(data=dataset,
                               experiment_name='experiment_tn_old_label_median',
                               return_views=view,
                               nb_repetitions=15,
                               saving_rep=saving_repository)
            if dataset.find("zero") != -1:
                run_experiment(data=dataset,
                               experiment_name='experiment_tn_old_label_zero',
                               return_views=view,
                               nb_repetitions=15,
                               saving_rep=saving_repository)
...
def main_run_experiments_majority_vote_new_labels():
    for view in return_views:
        for dataset in datasets_new_labels:
            if dataset.find("mean") != -1:
                run_experiment_majority_vote(data=dataset,
                                             experiment_name='experiment_majority_vote_tn_new_label_mean',
                                             return_views=view,
                                             nb_repetitions=15,
                                             saving_rep=saving_repository)
            if dataset.find("median") != -1:
                run_experiment_majority_vote(data=dataset,
                                             experiment_name='experiment_majority_vote_tn_new_label_median',
                                             return_views=view,
                                             nb_repetitions=15,
                                             saving_rep=saving_repository)
            if dataset.find("zero") != -1:
                run_experiment_majority_vote(data=dataset,
                                             experiment_name='experiment_majority_vote_tn_new_label_zero',
                                             return_views=view,
                                             nb_repetitions=15,
                                             saving_rep=saving_repository)
...
def main_run_experiments_majority_vote_old_labels():
    for view in return_views:
        for dataset in datasets_old_labels:
            if dataset.find("mean") != -1:
                run_experiment_majority_vote(data=dataset,
                                             experiment_name='experiment_majority_vote_tn_old_label_mean',
                                             return_views=view,
                                             nb_repetitions=15,
                                             saving_rep=saving_repository)
            if dataset.find("median") != -1:
                run_experiment_majority_vote(data=dataset,
                                             experiment_name='experiment_majority_vote_tn_old_label_median',
                                             return_views=view,
                                             nb_repetitions=15,
                                             saving_rep=saving_repository)
            if dataset.find("zero") != -1:
                run_experiment_majority_vote(data=dataset,
                                             experiment_name='experiment_majority_vote_tn_old_label_zero',
                                             return_views=view,
                                             nb_repetitions=15,
                                             saving_rep=saving_repository)
