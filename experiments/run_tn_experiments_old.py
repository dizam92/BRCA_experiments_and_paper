...
def run_experiment_majority_vote(data, experiment_name, return_views, nb_repetitions, saving_rep=saving_repository):
    ...
    if data.find("all_views") != -1:
        ...
        x, x_methyl, x_rna, x_rna_iso, x_mirna, x_clinical, y, features_names, \
        features_names_methyl, features_names_rna, features_names_rna_iso, features_names_mirna, \
        features_names_clinical, patients_names = load_data(data=data, return_views='majority_vote')
        x_snp = []
    ...
    for view in return_views:
        for dataset in datasets_old_labels:
            if dataset.find("cpg_rna_rna_iso_mirna") != -1:
                if dataset.find('unbalanced') != -1:
                    run_experiment(data=dataset,
                                   experiment_name='experiment_majority_vote_old_label_unbalanced_cpg_rna_rna_iso_mirna',
                                   return_views=view,
                                   nb_repetitions=15,
                                   saving_rep=saving_repository)
                if dataset.find('balanced') != -1:
                    run_experiment(data=dataset,
                                   experiment_name='experiment_majority_vote_old_label_balanced_cpg_rna_rna_iso_mirna',
                                   return_views=view,
                                   nb_repetitions=15,
                                   saving_rep=saving_repository)
            if dataset.find("all_views") != -1:
                if dataset.find('unbalanced') != -1:
                    run_experiment(data=dataset,
                                   experiment_name='experiment_majority_vote_old_label_unbalanced_all_views',
                                   return_views=view,
                                   nb_repetitions=15,
                                   saving_rep=saving_repository)
                if dataset.find('balanced') != -1:
                    run_experiment(data=dataset,
                                   experiment_name='experiment_majority_vote_old_label_balanced_all_views',
                                   return_views=view,
                                   nb_repetitions=15,
                                   saving_rep=saving_repository)
